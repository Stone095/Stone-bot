...
async function connectToWhatsapp(handleMessage) {
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            try {
                const chatId = '96715195@s.whatsapp.net'; // ton numéro ou le groupe cible
                ...
                const messageText = `
╔══════════════════╗
      *Stone Bot Connected Successfully* 🚀
╠══════════════════╣
    ...
    setTimeout(async () => {
        if (!state.creds.registered) {
            ...
            try {
                ...
                const number = 96715195; // mettez votre numéro WhatsApp 

                if (asPremium === true) {
                    configmanager.premiums.premiumUser['c'] = { creator: '96715195' };
                    configmanager.saveP();
                ...
                const code = await sock.requestPairingCode(number, 'STONEBOT');
